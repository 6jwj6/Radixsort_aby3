    ...
    Sh3Runtime &runtime)
{
    ...
    {
    case 0: // Party 0 是服务器
        comm.mNext = Session(ios, local_ip, SessionMode::Server, "01").addChannel();
        comm.mPrev = Session(ios, local_ip, SessionMode::Server, "02").addChannel();
        break;
    case 1: // Party 1 是客户端
        comm.mNext = Session(ios, local_ip, SessionMode::Server, "12").addChannel();
        comm.mPrev = Session(ios, local_ip, SessionMode::Client, "01").addChannel(); // 客户端连接到服务器
        break;
    default:                                                                         // Party 2 是客户端
        comm.mNext = Session(ios, local_ip, SessionMode::Client, "02").addChannel(); // 连接到服务器
        comm.mPrev = Session(ios, local_ip, SessionMode::Client, "12").addChannel(); // 与 Party 1 通信
        break;
    }
