...
i64 Sh3_Int_Vector_test(u64 n)
{
    ...
    vector<vector<u32>> pais(3, vector<u32>(n));
    foru(i, 0, 3)
    {
        vector<u32> round_perm(n);
        std::iota(round_perm.begin(), round_perm.end(), 0);
        std::random_shuffle(round_perm.begin(), round_perm.end(), prng);
        pais[i] = round_perm;
    }
    vector<i64> zeroshares(3);
    zeroshares[0] = prng.get<i64>();
    zeroshares[1] = prng.get<i64>();
    zeroshares[2] = (i64)0 - zeroshares[0] - zeroshares[1];
    // i64 xx = zeroshares[0] + zeroshares[1] + zeroshares[2];
    // foru(i, 0, n) cout << round_perm[i] << " \n"[i == n - 1];
    // foru(i, 0, 3) cout << zeroshares[i] << " \n"[i == n - 1];
    // cout << "xx = " << xx << endl;

    auto routine = [&](int idx)
    {
        PRNG prng(ZeroBlock);
        vector<u32> pai_now = pais[idx], pai_nxt = pais[(idx + 1) % 3], inv_pai_now(n), inv_pai_nxt(n);
        foru(i, 0, n) inv_pai_now[pai_now[i]] = inv_pai_nxt[pai_nxt[i]] = i;
        i64 zeroshares_now = zeroshares[idx], zeroshares_nxt = zeroshares[(idx + 1) % 3];
        // cout << "idx=" << idx << endl;
        // cout << "pai_now : ";
        // foru(i, 0, n) cout << pai_now[i] << " \n"[i == n - 1];
        // cout << "pai_nxt : ";
        // foru(i, 0, n) cout << pai_nxt[i] << " \n"[i == n - 1];
        /*--------------Preparations---------------------------------------------------*/
        ...
        auto Random_pai = [&]() -> void
        {
            ...
            std::iota(round_perm.begin(), round_perm.end(), 0);
            std::random_shuffle(round_perm.begin(), round_perm.end(), prng);
            // cout << "idx = " << idx << "  round_perm : ";
            // foru(i, 0, n) cout << round_perm[i] << " \n"[i == n - 1];

            vector<u32> tmp1 = pai_now, tmp2 = pai_nxt;
            foru(i, 0, n) pai_now[round_perm[i]] = tmp1[i], pai_nxt[round_perm[i]] = tmp2[i];
            foru(i, 0, n) inv_pai_now[pai_now[i]] = inv_pai_nxt[pai_nxt[i]] = i;
            // cout << "idx=" << idx << "  cnt = " << cnt << "  flag = " << flag << endl;
            // cout << "pai_now : cnt = " << cnt << endl;
            // foru(i, 0, n) if (i > n - 10) cout << pai_now[i] << " \n"[i == n - 1];
            // cout << "pai_nxt : cnt = " << cnt << endl;
            // foru(i, 0, n) if (i > n - 10) cout << pai_nxt[i] << " \n"[i == n - 1];
        };
        ...
        auto ReshareVec = [&](si64Matrix &X, u32 i) -> void
        {
            ...
            i64 tmp = prng.get<i64>();
            zeroshares_now = zeroshares_now * tmp;
            zeroshares_nxt = zeroshares_nxt * tmp;
            if (idx == nxt || idx == pre)
            {
                foru(i, 0, n)
                {
                    X[0](i) = X[0](i) + zeroshares_nxt;
                    X[1](i) = X[1](i) + zeroshares_now;
                }
            }
            if (idx == i)
            {
