...
i64 Sh3_Int_Vector_test(u64 n)
{
    ...
    auto routine = [&](int idx)
    {
        ...
        auto ApplyPerm = [&](vector<si64> rho, vector<si64> k)
        {
            ...
            foru(i, 0, n) task = enc.revealAll(rt.noDependencies(), pai_rho[i], rho_inv_pai[i]);
            task.get();
            // rho_inv_pai[i] = enc.revealAll(comms[idx], pai_rho[i]);
            vector<si64> rho_k(n);
...
bit key
        ...
        foru(i, 0, n)
            ...
            task = (idx == 0) ? enc.localInt(rt.noDependencies(), a[i], A[i]) : enc.remoteInt(rt.noDependencies(), A[i]);
        ...
        foru(i, 0, m) foru(j, 0, n)
            task = (idx == 0) ? enc.localInt(rt.noDependencies(), k_plain[i][j], k[i][j]) : enc.remoteInt(rt.noDependencies(), k[i][j]);
        task.get();
