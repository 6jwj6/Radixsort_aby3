...
i64 Sh3_Int_Vector_test(u64 n)
{
    ...
    auto routine = [&](int idx)
    {
        ...
        sb64 sone, szero;
        sone = idx == 0 ? enc.localBinary(comms[idx], 1) : enc.remoteBinary(comms[idx]);
        szero = idx == 0 ? enc.localBinary(comms[idx], 0) : enc.remoteBinary(comms[idx]);
        ...
        vector<sbMatrix> k(m, sbMatrix(n, 64));
        foru(i, 0, m)
            task = idx == 0 ? enc.localBinMatrix(rt.noDependencies(), k_plain[i], k[i]) : enc.remoteBinMatrix(rt.noDependencies(), k[i]);
        task.get();
        TimePoint("input");
        cout << idx << "before\n";
        auto x = A[0];
        ...
        sb64 z;
        task = idx == 0 ? enc.localBinary(rt.noDependencies(), 23, z) : enc.remoteBinary(rt.noDependencies(), z);
        task.get();
        auto zz1 = z.mData[0], zz2 = z.mData[1];
        cout << "idx = " << idx << endl;
        cout << zz1 << endl
             << zz2 << endl;
        www2 ^= zz1;
